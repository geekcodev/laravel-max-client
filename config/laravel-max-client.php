...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Глобальный token bucket 30 req/s на весь API (platform-api2.max.ru),
    | применяется к каждому запросу, включая ретраи и загрузку медиа.
    | При исчерпании запрос ожидает пополнения бакета (ядро HttpClient).
    |
    */

    'global_rate_limit' => [
        'tokens_per_second' => (float) env('MAX_GLOBAL_RATE_LIMIT_TOKENS_PER_SECOND', 30.0),
        'max_tokens' => (float) env('MAX_GLOBAL_RATE_LIMIT_MAX_TOKENS', 30.0),
    ],
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Логирование HTTP-запросов/ответов (middleware max.log) и обработки
    ...
    | для остальных роутов (например, /webapp) подключай alias 'max.log'
